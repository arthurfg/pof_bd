montar_tabela_rendimento_uc_one <- function(df_pof_rendimento_base,
  ...
  if(indicador_rend == 1){
    ...
    if(tipo_rendimento %in% c(11,12,13,14)){
      pof_rendimento_grupo <- pof_rendimentox %>%
        ...
        group_by(ID_uc, Descricao_2, Descricao_3) %>%
  ...
  if(tipo_rendimento %in% c(11,12,13,14)){
    pof_final <- pof_rendimento_grupo %>%
      pivot_wider(names_from = c("Descricao_2", "Descricao_3"), values_from = valor) 
...
teste <- montar_tabela_rendimento_uc(df_pof_rendimento_base = "pof_rendimentos",
                          tipo_rendimento = c(0,1,11,12,13,14),
                          df_pof_morador_base = "df_morador")


teste %>% head(10000) %>% write.csv("teste_pedro3.csv")
